...
namespace Ddd.Domain.Exceptions
{
    ...
    {
        public ConcurrencyException(Type aggregateType, IAggregateIdentity id)
            : base(string.Format("A different version than expected was found in aggregate {0} {1}", aggregateType.FullName, id))
        {            
        }

        public ConcurrencyException(Type aggregateType, IAggregateIdentity id, Exception innerException)
